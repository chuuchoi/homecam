// app/routes/m/add-device/select-home.tsx
    ...
    <div className="w-full bg-[#121212] text-white min-h-screen pt-6">
      ...
      <main className="px-6 py-4">
        <div className="relative w-full gap-4 flex flex-col">
          {homes.map((home, idx)=><label key={idx} htmlFor={`checkbox-${idx}`} className="flex justify-between bg-neutral-800 w-full px-3 py-2 items-center rounded">
            <div className="flex gap-2 items-center">
              <HouseIcon className="w-8 h-8" />
              <span className="mt-0.5">{home.name}</span>
            </div>
            <Checkbox
              id={`checkbox-${idx}`}
          ...
          onClick={()=>{setIsAdding(true)}}> + 새 홈 추가</div>
          ...
          <footer className="fixed bottom-0 left-0 w-full py-3 px-6">
              <button
                ...
                onClick={()=>{
                  if(selected===null) return alert("홈을 선택해주세요")
                  navigate(`/m/add-device/turn-on-device?home=${homes[selected].name}&mid=${mid}`)
                }}
              >
                다음
              </button>
      ...
      {isAdding && <div className="fixed top-0 left-0 bg-black/50 w-full h-full flex items-center justify-center">
        <div className="relative bg-neutral-800 rounded-xl gap-2 p-4 border border-neutral-700 flex flex-col items-center justify-center">
          ...
          <div className="flex gap-2 self-end">
            ...
            onClick={()=>{setHomes([...homes, {name}]);setIsAdding(false);setName("")}}>추가</button>
          </div>
        </div>
      </div>}
    </div>
