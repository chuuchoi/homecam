...
const DETECT_TYPE_OPTIONS_MAP: Record<string, string> = {
  '모션': 'motion',
  '소리': 'sound'
}
const DETECT_OBJ_OPTIONS_MAP: Record<string, string> = {
  '사람': 'person',
  '동물': 'animal'
}
...
export const SearchFilters = ({ filter, setFilter, fixedDetectType, fixedDetectObj }:
  ...
  const handleSelectDetectType = (index: number) => {
    setFilter((c: FilterT) => ({ ...c, detectType: DETECT_TYPE_OPTIONS_MAP[DETECT_TYPE_OPTIONS[index]] }));
  };
  const handleSelectDetectObj = (index: number) => {
    setFilter((c: FilterT) => ({ ...c, detectObj: DETECT_OBJ_OPTIONS_MAP[DETECT_OBJ_OPTIONS[index]] }));
  };
