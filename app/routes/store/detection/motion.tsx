...
import { ModelList, SearchBar, SearchFilters } from "../category/all";
import dummy from "../dummyModels.json";
export function meta({}) {
  return [
    { title: "스토어" },
    { name: "description", content: "Store > Event Detection > Motion Detection Model" },
  ];
}

const DUMMY_DATA = dummy.filter(d=>d.detect_type==='motion')
  ...
  return(
    <main className="bg-black text-white min-h-[calc(100vh-441px)] px-6 pt-2 pb-6 max-w-[1520px]">
      <div className="bg-black py-4 sticky top-[77px] mb-2">
        <h1 className="float-left">모션감지 모델</h1>
